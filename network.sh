...
function generatePeerArtifacts() {
    ...
    # docker-compose.yaml
    sed -e "s/PEER_EXTRA_HOSTS/$peer_extra_hosts/g" -e "s/CLI_EXTRA_HOSTS/$cli_extra_hosts/g" -e "s/API_EXTRA_HOSTS/$api_extra_hosts/g" -e "s/DOMAIN/$DOMAIN/g" -e "s/\([^ ]\)ORG/\1$org/g" -e "s/API_PORT/$api_port/g" -e "s/WWW_PORT/$www_port/g" -e "s/CA_PORT/$ca_port/g" -e "s/PEER0_PORT/$peer0_port/g" -e "s/PEER0_EVENT_PORT/$peer0_event_port/g" -e "s/PEER1_PORT/$peer1_port/g" -e "s/PEER1_EVENT_PORT/$peer1_event_port/g" ${compose_template} -e "s/COUCHDB0_PORT/$couchdb0_port/g" -e "s/COUCHDB1_PORT/$couchdb1_port/g" -e "s/COUCHDB_USER=/COUCHDB_USER=$COUCHDB_USER/g" -e "s/COUCHDB_PASSWORD=/COUCHDB_PASSWORD=$COUCHDB_PASSWORD/g" -e "s/CORE_LEDGER_STATE_COUCHDBCONFIG_USERNAME=/CORE_LEDGER_STATE_COUCHDBCONFIG_USERNAME=$COUCHDB_USER/g" -e "s/CORE_LEDGER_STATE_COUCHDBCONFIG_PASSWORD=$COUCHDB_PASSWORD/g"| awk '{gsub(/\[newline\]/, "\n")}1' > ${f}
